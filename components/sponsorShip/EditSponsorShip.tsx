...
const EditSponsorship = () => {
     ...
     useEffect(() => {
          if (sponsorData) {
               reset({
                    personName: sponsorData?.personName || "",
                    businessName: sponsorData?.businessName || "",
                    suburb: sponsorData?.suburb || "",
                    serviceType: sponsorData?.serviceType || "",
                    logo: sponsorData?.logo || "",
                    contact: sponsorData?.contact || "",
               });

               setSelected(sponsorData?.suburb || "");
               setSelectedCat(
                    sponsorData?.serviceType || "Mortgage Broker"
               );

               setPreview(sponsorData?.logo || "");
          }
     }, [sponsorData, reset]);

     const { mutate, isPending } = useMutation({
          mutationFn: (data: CreateSponsor) => editSponsor(id, data),
             ...
             <div className="flex flex-col gap-2" ref={wrapperRef}>
               ...
               <div className="relative">
                 <button
                   type="button"
                   disabled
                   className="flex w-full cursor-not-allowed items-center justify-between rounded-lg border border-gray-200 bg-gray-100 px-4 py-3 text-sm text-gray-500"
                 >
                   <span>{selected || "Select suburb"}</span>
                 ...
                 {sponsor.map((opt) => {
                   ...
                   return (
                     <button
                       ...
                       disabled
                       key={opt?.value}
                       className={`flex cursor-not-allowed items-center gap-2 rounded-lg border px-4 py-2.5 text-sm transition ${
                         isSelected
                           ? "border-bg-primary bg-bg-primary/10 text-bg-primary opacity-70"
                           : "border-border-primary bg-gray-100 text-gray-400"
                       }`}
                     >
                       <span
